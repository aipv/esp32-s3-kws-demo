...
void app_main(void)
{
    ...
    ESP_LOGI("KWS", "model=%s", model_name);
    ...
    ESP_LOGI("KWS", "samples=%d", samples);

    int32_t *raw_buffer =
        malloc(samples * sizeof(int32_t));

    int16_t *pcm_buffer =
        malloc(samples * sizeof(int16_t));
